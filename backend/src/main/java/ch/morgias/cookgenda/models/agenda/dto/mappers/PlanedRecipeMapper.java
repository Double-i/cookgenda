...
public interface PlanedRecipeMapper {
    ...
    default Collection<PlaningDayDto> toPlanedRecipeDtoV2List(Map<Long, List<PlanedRecipe>> planedRecipes){
        List<PlaningDayDto> r = new ArrayList<>();
        for(Map.Entry<Long, List<PlanedRecipe>> planedRecipe : planedRecipes.entrySet()){
            LocalDate date = LocalDate.ofEpochDay(planedRecipe.getKey());
            PlaningDayDto dto = new PlaningDayDto();
            dto.setDate(date.atStartOfDay());
            dto.setRecipes(planedRecipe.getValue().stream().map(INSTANCE::toPlanedRecipeDtoV2).toList());
        }
        return r;
    }
}
